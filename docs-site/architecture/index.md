...
| **工程** | ruff · pytest · Vitest · GitHub Actions CI · VitePress |
...
└──────────────────────┘      └────────────────────────────┘
         ...
         ▼
┌──────────────────────┐      ┌────────────────────────────┐
│ notifier Worker      │─────▶│ Webhook 接收平台             │
│ HMAC · 租约 · 重试    │      │ 自动化 / IM / 自建服务        │
└──────────────────────┘      └────────────────────────────┘
```
...
- **外部集成**：业务事务同时写入领域事件和待投递记录；独立 notifier 领取数据库租约后执行 HMAC 签名、SSRF 复检、指数退避和结果落库
