...
async def _get_member_role(project_id: int, user: User, db: AsyncSession) -> str | None:
    """Return role string or None for superuser/owner bypass."""
    member = await ensure_project_member(project_id, user, db)
    if user.is_superuser:
        return None
    if member:
        return member.role
    # Owner without membership row
...
async def create_task(
    ...
    result = await db.execute(
        select(func.max(Task.order)).where(Task.status_id == data.status_id)
    )
...
async def add_subtask(
    ...
    result = await db.execute(
        select(func.max(Task.order)).where(Task.status_id == parent.status_id)
    )
    max_order = result.scalar() or 0
    subtask = Task(
        project_id=project_id,
